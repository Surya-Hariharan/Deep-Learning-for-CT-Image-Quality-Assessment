"""One-off generator for notebooks/01_eda_ldct_iqac.ipynb.
...
Code-only notebook (no markdown cells, per project convention): section
banners are printed at runtime instead of written as markdown.
"""
...
# 1. Imports and repo-root resolution
code(
    """import json
import os
...
# 3. Load and inspect the JSON label files directly
code(
...
# 4. List image files on disk and compare extensions found
code(
...
# 5. Cross-check: every image has a label, every label has an image
code(
...
# 6. Inspect raw image properties (mode, size, dtype, pixel range)
code(
...
# 7. Duplicate image check (exact pixel match, via hash)
code(
    """import hashlib
...
# 8. Score distribution statistics
code(
...
# 9. Score distribution histograms (train vs test)
code(
...
# 10. Visualize a grid of sample training images with their scores
code(
...
# 11. Visualize a grid of sample testing images with their scores
code(
    """show_sample_grid(TEST_IMAGE_DIR, test_labels, "Testing samples (raw 512x512)")"""
)

# 12. Visualize images across the score range (low/mid/high quality)
code(
...
out_path = Path("notebooks/01_eda_ldct_iqac.ipynb")
