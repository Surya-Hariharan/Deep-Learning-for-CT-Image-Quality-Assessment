...
configuration source -- see docs/repository_architecture_audit.md,
...
def test_default_dataset_paths_use_canonical_locations():
    ...
    docs/repository_architecture_audit.md, "Current Live Bug")."""
...
def test_from_yaml_files_ignores_documentation_only_keys(tmp_path):
    """dataset.yaml's `name` field documents the dataset but isn't a dataclass field."""
    (tmp_path / "dataset.yaml").write_text(yaml.safe_dump({"name": "ldct_iqac"}))
    for fn in ("preprocessing.yaml", "model.yaml", "training.yaml"):
