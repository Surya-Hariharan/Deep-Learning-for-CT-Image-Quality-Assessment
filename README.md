...
**Updated 2026-09-13:** weight *loading* is now verified on this machine --
265/318 backbone keys matched, 0 unexpected, 53 non-learned
`num_batches_tracked` counters "missing" as expected -- and
`configs/model.yaml`'s `radimagenet_weights_path` points at the converted
file. See `weights/pretrained/radimagenet/resnet50/README.md` for the full
verification record. **What is still true:** no full training run using
these weights has been executed and persisted in this repository -- a
verified-loadable checkpoint is not the same as a trained result, and this
is reported honestly wherever it matters (see
`docs/replication/deviations.md`), never silently glossed over.

## 11. Training

Before training, `notebooks/02_data_preparation/01_dataset_preparation.ipynb`
(generated by `tools/build_dataset_preparation_notebook.py`) validates the
full `raw file -> preprocessing -> model-ready tensor -> normalized target`
path against the real dataset -- integrity, score distribution, train/test
leakage, and the exact tensor shapes/ranges the model receives -- and ends
with an explicit `DATA PIPELINE STATUS: READY` (or `NOT READY`) line.
...
`configs/*.yaml`, builds the dataset/model, runs a 2-batch **smoke test**
(forward/backward/`optimizer.step()`, gradients checked, then weights
restored so it doesn't affect the real run), and runs the training loop
...
**Data pipeline: verified READY** (dataset integrity, preprocessing,
tensor shapes, target normalization, and zero train/test leakage all
confirmed against the real dataset --
`notebooks/02_data_preparation/01_dataset_preparation.ipynb`).
**RadImageNet weight loading: verified** (265/318 keys matched, 0
unexpected -- see `weights/pretrained/radimagenet/resnet50/README.md`).
**Full training: not yet run.** `experiments/001_resnet50_baseline/`
currently contains only configuration scaffolding (`README.md`,
`config.yaml`, and a `config.json` from the smoke-test notebook run), no
...
`docs/decisions/README.md`. The pipeline itself is implemented, tested
end-to-end, and has passed a real-data smoke test (forward, backward,
optimizer step -- see `docs/replication/reproducibility.md`), but no
scientific result should be read out of this repository until a full
30-epoch run is actually executed.
