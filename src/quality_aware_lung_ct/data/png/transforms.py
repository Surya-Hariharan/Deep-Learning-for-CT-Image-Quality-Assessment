"""PNG dataset transform pipeline.

Migrated unmodified from the author's `utils/transforms.py`. `LoadImage`
now reads a caller-resolved path (see `metadata.resolve_series_path`),
so it makes no assumption about `.nii` vs `.nii.gz`.
"""
...
from ...preprocessing.volumes import crop_image, load_as_array, normalize_ct
...
def get_transforms(config: NGPNetConfig, is_test: bool = False):
    ''' Build train/val transforms (or a single test transform). '''
    SCOPE = (config.s_min, config.s_max)
    RANGE = (config.r_min, config.r_max)
    SHAPE = [config.in_size] * 3

    if not is_test:
        tra_transforms = Compose([
            LoadImage(img_dtype=np.float32, msk_dtype=np.uint8),
            RandomCrop(rand_crop=False, crop_size=SHAPE),
            ScaleIntensity(scope=SCOPE, range=RANGE),
            RandomFilp(prob=0.2, axes=(0, 1, 2)),
            RandomRot90(prob=0.2, axes=(0, 1, 2)),
            RandomScaleIntensity(prob=0.1, factor=0.1),
            RandomShiftIntensity(prob=0.1, offset=0.1*(RANGE[1]-RANGE[0])),
            AddChannel(img_add=True, msk_add=True),
            ToTensor()
        ])
        ...
        return (tra_transforms, val_transforms)
    else:
