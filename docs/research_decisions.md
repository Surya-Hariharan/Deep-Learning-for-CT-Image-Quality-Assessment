...
| A-23 | Input normalization baseline = `tensorflow.keras.applications.resnet50.preprocess_input` (Keras's own "caffe"-mode preprocessing: RGB→BGR channel reorder, then per-channel mean subtraction using ImageNet's channel means, applied to float32 pixel values still on the raw `[0,255]` scale — **no** additional `/255` rescale). Resolves U-M05. Classification: **UNKNOWN → PROJECT-ADAPTATION baseline** (explicitly not OHASHI-SPECIFIED, not a confirmed RadImageNet convention either — see rationale). | **Investigated systematically, 2026-08-14** (`docs/training_environment.md` normalization section has the full trace): (A) Ohashi's paper was never obtained (as with every OHASHI-SPECIFIED item in this file) — no normalization statement is available from it, full stop; the project brief this repo was built from is likewise silent. (B) No Ohashi supplementary material or source code was found publicly. (C) The RadImageNet paper (Mei et al. 2022) states only "All images were resized to 224×224 pixels and used as the inputs" for their own pretraining, and "downscaled to 256×256" for their downstream experiments — **no pixel-scaling, mean-subtraction, or normalization scheme is stated anywhere in the paper**, for either their base pretraining or their downstream fine-tuning. (D)/(E) The official `BMEII-AI/RadImageNet` repository's own downstream example scripts (`acl/acl_train.py`, `thyroid/thyroid_train.py` — checked both, same pattern in each) consistently use `tensorflow.keras.applications.imagenet_utils.preprocess_input` (default `mode="caffe"`, i.e. the same transform as `resnet50.preprocess_input`) as a Keras `ImageDataGenerator` `preprocessing_function` — **but combine it with an additional `rescale=1./255`, which is internally inconsistent**: `preprocess_input(mode="caffe")`'s hardcoded mean constants (103.939, 116.779, 123.68) are calibrated for `[0,255]`-scale input, so applying them after an independent `1/255` rescale produces values dominated by a large near-constant negative offset rather than a principled normalization — this combination looks like a copy-paste artifact in the official repo's own example code, not a deliberately validated recipe, and is called out here rather than silently adopted. (F) No RadImageNet *base-pretraining* script (the code that actually produced this checkpoint, as opposed to downstream fine-tuning examples) is published in the repository — everything under (D)/(E) is evidence about how a third party's *downstream* code *uses* the checkpoint, not about how the checkpoint's own training data was normalized. (G) The checkpoint file itself (`data/external/radimagenet/RadImageNet-ResNet50_notop.h5`) was inspected directly via `h5py` (not inferred from weight values, per instruction): its embedded `model_config` shows the saved architecture is `InputLayer -> ZeroPadding2D -> Conv2D -> ...` with **no** `Rescaling`/`Normalization`/`Lambda` preprocessing layer anywhere in its 175 saved layers — the checkpoint provides **zero** structural evidence resolving this question either way. **Decision**: adopt the *corrected* form of the one repeated, if imperfect, signal available — `preprocess_input`'s BGR-reorder-plus-mean-subtraction on raw `[0,255]` input, dropping the redundant `/255` rescale that made the official example code internally inconsistent. This is a REFERENCE-IMPLEMENTATION-CONVENTION-*informed* choice (repeatedly used, if flawed, in the checkpoint's own downstream examples) adopted as this project's own PROJECT-ADAPTATION — explicitly *not* claimed to be what Ohashi did, and *not* claimed to be a verified statement of how the checkpoint's own base training was normalized. **Not chosen by any hyperparameter search or validation comparison**, per this decision's own scope constraint (same constraint as A-22). **Alternatives considered and rejected:** (a) raw `[0,255]`, no normalization at all — rejected because it discards the one piece of repeated, if imperfect, RadImageNet-side evidence entirely; (b) `[0,1]` rescale only, no mean-subtraction — a defensible, more conservative alternative that introduces the ImageNet-specific mean constants; genuinely close to (a) in reasoning strength, and reasonable people could pick it instead — recorded here as the leading alternative, not dismissed as unreasonable; (c) copying the official scripts' exact `rescale=1/255` + `preprocess_input` combination verbatim — rejected because it is demonstrably internally inconsistent (see above), and reproducing a likely bug is not "following the reference implementation," it is reproducing its mistake. |
...
| U-M05 | ~~RadImageNet preprocessing / intensity normalisation~~ — **RESOLVED, see A-23** | differs from standard ImageNet preprocessing | Resolved 2026-08-14 as UNKNOWN → PROJECT-ADAPTATION baseline (decision A-23): `tensorflow.keras.applications.resnet50.preprocess_input` on raw `[0,255]` input. Neither Ohashi's paper nor RadImageNet's own paper specifies this; RadImageNet's official downstream example code was consulted and found to use an internally-inconsistent combination, corrected rather than copied verbatim — see A-23 for the full trace. |
...
## Preprocessing Specification Decision (2026-08-14, second follow-up)

Following checkpoint acquisition and isolated-environment provisioning
(`docs/training_environment.md`), the last open baseline-preprocessing
item is resolved:

- **Input normalization** — decision A-23 above. UNKNOWN → PROJECT-ADAPTATION
  baseline: `tensorflow.keras.applications.resnet50.preprocess_input` on
  raw `[0,255]`-scale pixel values, no additional rescale. Systematically
  investigated against Ohashi's paper (unavailable), the RadImageNet
  paper (silent on normalization), the official RadImageNet repository's
  downstream example code (found and corrected for an internal
  inconsistency rather than copied), and the checkpoint file itself
  (inspected structurally via `h5py` — no preprocessing layer embedded,
  no evidence either way). Full trace in A-23 and
  `docs/training_environment.md`.
- The complete, authoritative baseline preprocessing specification —
  central crop (no resize), channel replication, `[0,255]` scale,
  `preprocess_input` normalization, output `(224, 224, 3)` float32 — is
  recorded in `docs/training_environment.md`, "Preprocessing
  Specification," and implemented/tested in
  `src/ct_iqa/preprocessing/normalization.py` and
  `tests/test_preprocessing.py`.
- Not chosen by any hyperparameter search, validation comparison, or
  training run — same scope constraint as A-22.
- This does **not** authorize `build_model()`, training, or production
  dataset generation. It resolves the last preprocessing unknown recorded
  as U-M05.
